...
| `needs_render` | STL exists (from local CAD source, or from an imported/scanned mesh with no local CAD source at all); no render exists yet. |
| `slicer_review_ready` | STL + renders exist and the preview package reports no missing/stale artifacts. Same meaning as the existing `slicer_review_ready` project status: ready for **human** slicer review, not print-ready. |
| `blocked_or_incomplete` | Something doesn't parse (corrupt `brief.json`/`part_manifest.json`), or files exist but the preview package still flags missing/stale artifacts. |
