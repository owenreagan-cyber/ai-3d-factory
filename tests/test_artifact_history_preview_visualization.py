"""Phase 41 tests: wiring `artifact_history_summary` into the Preview Board
...
"Artifact History" card in the Preview Board HTML. See
docs/artifact-history.md, docs/preview-board.md, docs/roadmap.md Phase 41.
"""
...
def test_gather_board_data_includes_artifact_history_summary(isolated_projects_dir):
    # A bare project (brief only, no CAD generation yet) has no
    # artifact-relevant timeline events at all.
    project_store.init_project("Bare Project")
    board = gather_board_data(isolated_projects_dir)
    project = board["projects"][0]
    assert "artifact_history_summary" in project
    assert project["artifact_history_summary"]["history_available"] is False


def test_gather_board_data_reflects_export_versions(isolated_projects_dir, scad_project, monkeypatch):
    _export_all(scad_project, monkeypatch)
    board = gather_board_data(isolated_projects_dir)
    project = board["projects"][0]
    assert project["artifact_history_summary"]["history_available"] is True
    assert project["artifact_history_summary"]["version_count"] >= 1
...
# ---- Preview Board HTML: Artifact History card ----


def test_html_includes_artifact_history_section_heading(isolated_projects_dir, scad_project):
    board = gather_board_data(isolated_projects_dir)
    html = build_board_html(board)
    assert "Artifact History" in html


def test_html_artifact_history_appears_after_project_timeline_before_project_intake(
    ...
    artifact_index = html.find("<h4>Artifact History</h4>")
    intake_index = html.find("<h4>Project Intake</h4>")
    assert timeline_index != -1
    assert artifact_index != -1
    assert intake_index != -1
    assert timeline_index < artifact_index < intake_index


def test_html_artifact_history_no_versions_message(isolated_projects_dir):
    project_store.init_project("Bare Project")
    board = gather_board_data(isolated_projects_dir)
    html = build_board_html(board)
    start = html.find("<h4>Artifact History</h4>")
    end = html.find("<h4>Project Intake</h4>", start)
    section = html[start:end]
    assert "No artifact versions recorded yet for this project." in section


def test_html_artifact_history_shows_version_count_and_latest(isolated_projects_dir, scad_project, monkeypatch):
    _export_all(scad_project, monkeypatch)
    board = gather_board_data(isolated_projects_dir)
    html = build_board_html(board)
    start = html.find("<h4>Artifact History</h4>")
    ...
    assert "Plan available" in section


def test_html_artifact_history_missing_summary_key_renders_cleanly():
    project = _full_project()
    assert "artifact_history_summary" not in project
    html = build_board_html(_minimal_board([project]))
    assert "No artifact versions recorded yet for this project." in html


def test_html_artifact_history_shows_none_for_no_recent_changes():
    project = _full_project(
        artifact_history_summary={
            ...
            "current_artifact_state": {
                "validation": "Not yet reached", "preview": "Not yet reached", "review": "Pending",
            },
        },
    )
    html = build_board_html(_minimal_board([project]))
    ...
    section = html[start:end]
    assert "Recent Changes" in section
    assert '<span class="none">None</span>' in section


def test_html_artifact_history_no_changed_since_previous_row_when_none():
    # `changed_since_previous` is `None` (not `[]`) when there's only one
    # version yet, so there's no "previous" to compare against at all -
    # the row should be omitted entirely, not shown as "None".
    project = _full_project(
        artifact_history_summary={
            "history_available": True,
            "version_count": 1,
    ...
    section = html[start:end]
    assert "Recent Changes" not in section


def test_html_artifact_history_shows_recent_changes_list():
    project = _full_project(
        artifact_history_summary={
            "history_available": True,
            "version_count": 2,
            "latest_version": 2,
            "changed_since_previous": ["CAD source", "STL file"],
            "current_artifact_state": {
                "validation": "Passed", "preview": "Rendered", "review": "Approved",
            },
        },
    )
    html = build_board_html(_minimal_board([project]))
    ...
    section = html[start:end]
    assert "CAD source, STL file" in section


def test_html_never_leaves_artifact_history_value_blank(isolated_projects_dir, scad_project, monkeypatch):
    _export_all(scad_project, monkeypatch)
    board = gather_board_data(isolated_projects_dir)
    html = build_board_html(board)
    start = html.find("<h4>Artifact History</h4>")
...
def test_existing_detail_cards_still_present_alongside_artifact_history_card(isolated_projects_dir, scad_project):
    board = gather_board_data(isolated_projects_dir)
    html = build_board_html(board)
...
def test_html_artifact_history_section_escapes_values():
    project = _full_project(
        artifact_history_summary={
            "history_available": True,
            "version_count": 2,
            "latest_version": 2,
            "changed_since_previous": ["<img src=x onerror=alert(1)>"],
            "current_artifact_state": {
                "validation": "Passed", "preview": "Rendered", "review": "Approved",
            },
        },
...
def test_html_artifact_history_section_has_no_external_assets_or_ai_calls():
...
def test_cli_preview_board_html_includes_artifact_history_section(isolated_projects_dir, scad_project):
...
def test_preview_board_json_includes_additive_artifact_history_summary(isolated_projects_dir, scad_project):
    result = runner.invoke(app, ["preview-board", str(isolated_projects_dir)])
    assert result.exit_code == 0, result.stdout
...
def test_review_gate_cli_unaffected_by_artifact_history(isolated_projects_dir, scad_project):
    result = runner.invoke(app, ["review-gate", "--json", str(scad_project)])
    payload = json.loads(result.stdout)
    assert "artifact_history_summary" not in payload


def test_slicer_readiness_cli_unaffected_by_artifact_history(isolated_projects_dir, scad_project):
    result = runner.invoke(app, ["slicer-readiness", str(scad_project), "--json"])
    payload = json.loads(result.stdout)
    assert "artifact_history_summary" not in payload


def test_timeline_cli_unaffected_by_artifact_history(isolated_projects_dir, scad_project):
